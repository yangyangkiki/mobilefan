...
def im_to_torch_1(img,mean,std):
...
def load_image_1(img,mean,std):
    # H x W x C => C x H x W

    return im_to_torch_1(img,mean,std)
...
# zhaoyang
def draw_vectormap(img, pt, sigma, vector_a,vector_b,tpts,type='Gaussian'):
    # Draw a 2D gaussian
    # Adopted from https://github.com/anewell/pose-hg-train/blob/master/src/pypose/draw.py
    img = to_numpy(img)
    ...
    bax = bax / norm_ba + 1e-9
    bay = bay / norm_ba + 1e-9

    # Check that any part of the gaussian is in-bounds
    ul = [int(pt[0] - 3 * sigma), int(pt[1] - 3 * sigma)]
    br = [int(pt[0] + 3 * sigma + 1), int(pt[1] + 3 * sigma + 1)]
    if (ul[0] >= img[0].shape[1] or ul[1] >= img[0].shape[0] or
            br[0] < 0 or br[1] < 0):
        # If not, just return the image as is
        return to_torch(img)
    ...
    # Usable gaussian range
    g_x = max(0, -ul[0]), min(br[0], img[0].shape[1]) - ul[0]
    g_y = max(0, -ul[1]), min(br[1], img[0].shape[0]) - ul[1]
    # Image range
    img_x = max(0, ul[0]), min(br[0], img[0].shape[1])
    img_y = max(0, ul[1]), min(br[1], img[0].shape[0])

    img[0][img_y[0]:img_y[1], img_x[0]:img_x[1]] = g_v_x[g_y[0]:g_y[1], g_x[0]:g_x[1]]
    img[1][img_y[0]:img_y[1], img_x[0]:img_x[1]] = g_v_y[g_y[0]:g_y[1], g_x[0]:g_x[1]]

    return to_torch(img)

def draw_vectormap_oritation(img, pt, sigma, vector_a,vector_b,tpts,type='Gaussian'):
    # Draw a 2D gaussian
    # Adopted from https://github.com/anewell/pose-hg-train/blob/master/src/pypose/draw.py
    img = to_numpy(img)
    ...
    return to_torch(img), (bax, bay)

# add vector:
def add_vector(target, tpts, nparts, vec_pair):
    for i in range(0, nparts):
        ...
        target[(2 * i):(2 * i + 2)][0] = target[(2 * i):(2 * i + 2)][0] * bax
        target[(2 * i):(2 * i + 2)][1] = target[(2 * i):(2 * i + 2)][1] * bay
    return target
