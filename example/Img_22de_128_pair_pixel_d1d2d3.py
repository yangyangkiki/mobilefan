...
def main(args):
    ...
    criterion_teather_pair = criterion_pair.CriterionSDcos_no().cuda() # CriterionSDcos_sig
...
def train(train_loader, model,model_teacher, criterion,criterion_teather_pixel,criterion_teather_pair, optimizer,epoch, debug=False, flip=True):
    ...
    for i, (inputs, target, meta) in enumerate(train_loader):
        ...
        with torch.no_grad():
            output_soft, teacher_deconv_features = model_teacher(input_var)

        loss_soft_pixel_2 = criterion_teather_pixel(pixel_features[2], teacher_deconv_features[2])
        loss_soft_pixel_1 = criterion_teather_pixel(pixel_features[1], teacher_deconv_features[1])
        loss_soft_pixel_0 = criterion_teather_pixel(pixel_features[0], teacher_deconv_features[0])
        loss_soft_pixel = loss_soft_pixel_0 + loss_soft_pixel_1 + loss_soft_pixel_2

        loss_soft_pair_2 = criterion_teather_pair(pair_features[2], teacher_deconv_features[2])
        loss_soft_pair_1 = criterion_teather_pair(pair_features[1], teacher_deconv_features[1])
        loss_soft_pair_0 = criterion_teather_pair(pair_features[0], teacher_deconv_features[0])
        loss_soft_pair = loss_soft_pair_0 + loss_soft_pair_1 + loss_soft_pair_2

        loss_soft = T_pixel * loss_soft_pixel + T_pair * loss_soft_pair
...
def evaluate(val_dataset,val_loader, model,model_teacher, criterion,criterion_teather_pixel,criterion_teather_pair, num_classes, debug=False, flip=True):
    ...
    for i, (inputs, target, meta) in enumerate(val_loader):
        ...
        # generate predictions
        preds = final_preds(score_map, meta['center'], meta['scale'], [64, 64])
        preds = final_preds(score_map_teacher, meta['center'], meta['scale'], [64, 64])
